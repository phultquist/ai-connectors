#!/usr/bin/env node
/**
 * Onboard a 7shifts customer in one command.
 *
 *   node scripts/onboard.mjs \
 *     --name "Acme Restaurant Group" \
 *     --token <their 7shifts API key> \
 *     [--company-id 435911] [--api https://api.aiconnectors.ai]
 *
...
const API = (flag("api", process.env.CONNECTOR_API ?? "https://api.aiconnectors.ai")).replace(/\/+$/, "");
...
if (!name || !token) {
  console.error(
    'Usage: node scripts/onboard.mjs --name "Acme Group" --token <7shifts-api-key> [--company-id 123]',
  );
  process.exit(1);
}
...
1. Add your key to the shell environment (and reopen the terminal):
     ...
     echo 'export SEVENSHIFTS_CONNECTOR_KEY=${key}' >> ~/.zshrc
