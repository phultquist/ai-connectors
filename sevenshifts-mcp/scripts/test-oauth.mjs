...
if (!CUSTOMER_TOKEN) { console.error("Set SEVENSHIFTS_TOKEN."); process.exit(1); }

/* ---- provision a tenant ------------------------------------------- */
const created = await (await call("/admin/tenants", {
  method: "POST",
  headers: { Authorization: `Bearer ${env.ADMIN_SECRET}`, "Content-Type": "application/json" },
  body: JSON.stringify({ name: "OAuth Test Co", sevenshifts_token: CUSTOMER_TOKEN }),
})).json();
...
console.log("\nUsing the access token (live 7shifts)");
{
  ...
  check("tools/list works with OAuth token", tools.result?.tools?.length === 28, `got ${tools.result?.tools?.length}`);

  const who = await (await rpc(accessToken, {
    jsonrpc: "2.0", id: 2, method: "tools/call",
    params: { name: "sevenshifts_whoami", arguments: {} },
  })).json();
  check("live call resolves the tenant's 7shifts account",
    (who.result?.content?.[0]?.text ?? "").includes("company_id"));
...
console.log("\nRegression: connector key still works");
{
  ...
  check("ck_live_ bearer auth unchanged", r.status === 200 && body.result?.tools?.length === 28, `got ${r.status}`);
}
