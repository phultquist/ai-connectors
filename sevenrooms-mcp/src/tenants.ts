 ...
 *
 * A customer never sees or sends their SevenRooms token to Claude. Instead we
 * provision them a connector key (`ck_live_...`) that Claude sends as a bearer
 * token; the Worker exchanges it for that tenant's SevenRooms token server-side.
 *
...
/** Public view — never includes the SevenRooms token. */
export type TenantSummary = Omit<TenantRecord, "sevenrooms_token"> & {
  sevenrooms_token: "[stored]";
};
...
export class TenantStore {
  ...
  async create(input: {
    ...
    if (!input.sevenrooms_token?.trim()) {
      throw new Error("A SevenRooms access token is required.");
    }
    ...
    const record: TenantRecord = {
      ...
      sevenrooms_token: input.sevenrooms_token.trim(),
...
function redact(record: TenantRecord): TenantSummary {
  return { ...record, sevenrooms_token: "[stored]" };
}
